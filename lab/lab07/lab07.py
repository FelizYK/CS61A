...
class Person:
    ...
    def __init__(self, name):
        self.name = name
        "*** YOUR CODE HERE ***"
        self.pre_sentence = 'I squirreled it away before it could catch on fire.'

    def say(self, stuff):
        "*** YOUR CODE HERE ***"
        self.pre_sentence = stuff
        return stuff
...
class Round:
    ...
    def play(self, who, card):
        ...
        if card >= self.highest:
            ...
            self.cur_winner = who
        if self.is_complete():
            self.winner = self.cur_winner

    def is_complete(self):
        """ Checks if a game could end. """
        return self.highest > 0 and self.starter == self.next_player
