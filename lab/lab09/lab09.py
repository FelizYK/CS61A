...
def is_bst(t):
    ...
    def bst_max(t):
        if t.is_leaf():
            return t.label
        m = max(t.branches, key=bst_max).label
        return max(t.label, m)
    
    def bst_min(t):
        if t.is_leaf():
            return t.label
        m = min(t.branches, key=bst_min).label
        return min(t.label, m)
    ...
    if t.is_leaf():
        ...
        if t.branches[0].label <= t.label and t.branches[1].label > t.label:
            return is_bst(t.branches[0]) and bst_max(t.branches[0]) <= t.label and is_bst(t.branches[1]) and bst_min(t.branches[1]) > t.label
    return False
