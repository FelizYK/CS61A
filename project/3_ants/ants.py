...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not place.is_hive:
            if place.bees and self.lower_bound <= count <= self.upper_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        for bee in self.place.bees[:]:  # bees may die, so copy the list to iterate over all
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt does nothing each turn, but has a large amount of health."""
...
# The HungryAnt Class
class HungryAnt(Ant):
    """HungryAnt will eat a random Bee from its place and chew for 3 turns."""
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is a ThrowerAnt but waterproof and more costly."""
...
class LaserAnt(ThrowerAnt):
    """LaserAnt damages all Insects in front of it, 
    including Ants and even its ContainerAnt.
    The laser is weakened by 0.25 each place.
    Each time LaserAnt actually damages an Insect, 
    its laser's total damage goes down by 0.0625 (1/16).
    """
