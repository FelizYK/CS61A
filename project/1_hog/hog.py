...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    highest, number = 0, 0
    for i in range(1,11):
        current = make_averaged(roll_dice, total_samples)(i, dice)
        if current > highest:
            highest = current
            number = i
    return number
...
def final_strategy(score, opponent_score):
    ...
    - Instead of using a threshold, roll 0 whenever it would give you 
    more points on average than rolling 6.
    *** YOUR DESCRIPTION HERE ***
    ...
    if score > opponent_score:
        for i in range(3):
            if square_update(i, score, opponent_score, dice) >= 100:
                return i
    roll6 = make_averaged(roll_dice, 10)(6)
    return square_strategy(score, opponent_score, max(roll6, 12), 6)
    # END PROBLEM 12
